...
int main ()
{
  ...
  for(i=0;i<CLDCNT;++i)
	...
	if(i==0){
printf("%s_\n",buf);
	  if(dup2(p1[0],1)==-1)perror("can't dup\n");
	  if(dup2(p2[0],2)==-1)perror("can't dup\n");
	  execlp(buf,(char *)0);
	exit(0);
	}
	if(i==1){
	  if(dup2(p1[1],0)==-1)perror("can't dup on exit pipe\n");
printf("%s_\n",buf+arg2);
	  execlp(buf+arg2,(char *)0);
	exit(0);
	}
	if(dup2(p2[1],0)==-1)perror("can't dup on exit pipe\n");
printf("%s_\n",buf+arg3);
